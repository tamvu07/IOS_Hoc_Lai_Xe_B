...
open class AssistiveTouch: UIButton {
    ...
    func reactBounds(touches: Set<UITouch>) {
        ...
        if center.x + offsetX >= screen.width / 2 {
            self.center = CGPoint(x: screen.width - bounds.size.width / 2 - padding, y: center.y + offsetY)
        } else {
            self.center = CGPoint(x: bounds.size.width / 2 + padding, y: center.y + offsetY)
        }
        if center.y + offsetY >= screen.height - bounds.size.height  - bottomPadding - topPadding{
            self.center = CGPoint(x: center.x, y: screen.height - bounds.size.height - padding - bottomPadding - topPadding)
        } else if center.y + offsetY < bounds.size.height {
            self.center = CGPoint(x: center.x, y: bounds.size.height / 2 + padding )
        }
